...
void isdbt_power_on_test(void)
{
	...
	if(TRUE != hwPowerOn(MT65XX_POWER_LDO_VAST, VOL_1200,NMI_HW_DRVNAME))
	{
...
{
#ifndef SIM_PAD
        if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800,NMI_HW_DRVNAME))
        {
            NM_KMSG("[ISDBT] Fail to enable digital power\n");
            return -EIO;
        }      
...
#else
			 xx
         if(TRUE != hwPowerOn(MT65XX_POWER_LDO_VM12_2, VOL_1200,NMI_HW_DRVNAME))
#endif
        {
            NM_KMSG("[ISDBT] Fail to enable digital power\n");
            return -EIO;
...
void isdbt_gpio_power_off(void)
{
	...
	mt_set_gpio_out(NMI_POWER_PIN, 0);
        ...
        if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D2, NMI_HW_DRVNAME))
        {
            NM_KMSG("[ISDBT] Fail to enable digital power\n");
            return -EIO;
        }

         if(TRUE != hwPowerDown(MT65XX_POWER_LDO_VM12_2, NMI_HW_DRVNAME))
        {
            NM_KMSG("[ISDBT] Fail to enable digital power\n");
            return -EIO;
        }      
...
void isdbt_gpio_interrupt_register(void)
{
    ...
    NM_KMSG("<isdbt> isdbt_gpio_interrupt_register, (%d)[%d]\n",GPIO_DVT_EINT_PIN,CUST_EINT_DTV_NUM);

	// gpio
	mt_set_gpio_mode(GPIO_DVT_EINT_PIN, GPIO_DVT_EINT_PIN_M_EINT);
	mt_set_gpio_dir(GPIO_DVT_EINT_PIN, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_DVT_EINT_PIN, false);

	// irq register
	mt65xx_eint_registration(CUST_EINT_DTV_NUM, CUST_EINT_DTV_DEBOUNCE_EN, CUST_EINT_DTV_POLARITY, isdbt_irq_handler, 0);//punk 0->1
...
void isdbt_gpio_interrupt_unregister(void)
{
	...
	// gpio
	mt_set_gpio_mode(GPIO_DVT_EINT_PIN, GPIO_DVT_EINT_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_DVT_EINT_PIN, GPIO_DIR_IN);
...
static void isdbt_read_gpio()
{
    ...
    val = mt_get_gpio_in(GPIO_DVT_EINT_PIN);
    NM_KMSG("~~~~~ <isdbt> val=%d\n", val);
...
static int isdbt_ioctl(/*struct inode *inode, */struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	NM_KMSG("<isdbt> ioctl [1]: cmd=%x)\n", cmd);
 
//	if(_IOC_TYPE(cmd) != IOCTL_MAGIC) return -EINVAL;
	if(_IOC_TYPE(cmd) != IOCTL_MAGIC){
		NM_KMSG("<isdbt> ioctl magic error: cmd=%x)\n", cmd);
		return -EINVAL;
	}
